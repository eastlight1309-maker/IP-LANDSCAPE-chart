...
def generate_sample(n=600, seed=42, sep="; ", multiclass_format="sep",
    ...
    for i in range(n):
        ...
        rows.append({
            ...
            "기술 중분류": l2,
            "다중 기술분류": multi_str,
            # B축(응용처)·C축(재료) — A축과 상관된 별도 분류 체계 (교차 해석 케이스)
            "B축 대분류": ["모바일", "서버·HPC", "차량", "웨어러블"][
                (l1_idx + i) % (2 if l1_idx == 0 else 4)],
            "B축 중분류": ["스마트폰 AP", "HBM 스택", "ADAS 모듈", "헬스케어 센서",
                        "데이터센터 가속기"][(l1_idx * 2 + i) % 5],
            "C축 대분류": ["유기 소재", "금속 소재", "세라믹 소재"][
                (l2_idx + (i % 2)) % 3],
            "C축 중분류": ["에폭시 몰딩 컴파운드", "솔더", "구리 배선", "실리카 필러",
                        "폴리이미드"][(l2_idx + i) % 5],
