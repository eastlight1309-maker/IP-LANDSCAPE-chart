...
(function () {
  ...
  Views.manual = function (content) {
    ...
    section('🖱️ 차트 공통 기능',
      ...
      '<li><b>🗂️ 인사이트 보관함 → PPT</b>: 생성된 인사이트는 차트 이미지와 함께 작업별로 자동 저장됩니다 (기본은 현재 작업만 표시, 이전 작업은 버튼으로 선택). 항목을 골라 PPT 로 내려받으면 <b>1p 표지, 2p 목차, 이후 차트마다 [차트 전체 페이지 → 다음 페이지에 그 차트의 인사이트]</b> 순서로 들어가고, 카드에 차트가 여러 개면 전부 수록됩니다.</li>' +
