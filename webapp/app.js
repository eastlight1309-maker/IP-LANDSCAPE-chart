...
(function () {
  ...
  var Insight = (function () {
    function box(result, analysisName, description) {
      ...
      } else if (State.config && State.config.settings) {
        // LLM 인사이트 설정이 꺼져 있으면(기본값·다른 사용자가 끈 경우 포함)
        // 생성 버튼이 사라져 기능이 없어진 것처럼 보인다 — 숨기는 대신
        // 켜기 안내 버튼을 표시해 한 번의 클릭으로 복구할 수 있게 한다.
        var enBtn = Ui.el('<button class="btn small" title="AI(LLM) 인사이트 생성 기능이 ' +
          '꺼져 있어 차트별 생성 버튼이 숨겨진 상태입니다. 켜면 전체 사용자 공통으로 ' +
          '적용됩니다 (Settings → LLM 인사이트 설정과 동일).">🤖 AI 인사이트 켜기</button>');
        enBtn.addEventListener('click', function () {
          Api.post('/api/settings', { llm_insights_enabled: true }).then(function (resp) {
            State.config.settings = resp.settings;
            Ui.toast('AI 인사이트 생성 버튼이 켜졌습니다 — 각 차트 아래 "🤖 이 차트 ' +
              '인사이트"와 "LLM 인사이트 생성 (차트별)" 버튼이 표시됩니다 ' +
              '(전체 사용자 공통, Settings → LLM 설정에서 끌 수 있습니다).');
            Views.render(State.view);
          }).catch(errToast);
        });
        drills.appendChild(enBtn);
      }
  ...
  var PURPOSES = [
    { key: 'tech_trend', icon: '📈', label: '기술 동향 분석',
      ...
      primary: [
        { v: 'evolution', t: '기술 생애주기', why: '성장/성숙/쇠퇴/재부상 단계를 성숙도×모멘텀으로 직접 판정 — 목적에 가장 직결' },
        ...
        { v: 'evolution', t: '기술 전이 Sankey', why: '기술 간 이동 흐름 — 무게중심이 어디로 옮겨가는지' },
        { v: 'evolution', t: '기술×연도 버블', why: '대·중·소 분류별 투자 시점·규모를 셀 단위로 비교' }],
      secondary: [
    ...
    { key: 'competitor', icon: '🏢', label: '경쟁사 분석',
      ...
      primary: [
        { v: 'competitor', t: '출원인 포커스', why: '경쟁사의 집중 기술(주력) + 🆕 신규 진입 기술 + ★ 급부상 아이템을 한 화면에서 — 목적의 핵심 3질문에 직접 답함' },
        { v: 'evolution', t: '기술분류 동향', why: '경쟁사 선택 — 어떤 기술분류의 출원이 많은지 건수·연도 추이로 확인' },
        { v: 'evolution', t: '기술분류 트리', why: '경쟁사 선택 — 그 회사 포트폴리오의 대·중·소 분류 구성 전체를 면적으로 조망' },
        { v: 'evolution', t: '기술×연도 버블', why: '어떤 기술에 언제 진입해 얼마나 투자했는지 시점·규모 비교 (최대 3사 동시)' },
        { v: 'evolution', t: '신흥 기술 탐지 (임베딩)', why: '경쟁사 선택 — 분류에 아직 안 잡힌 새 연구 주제(신규 분야) 조기 포착' },
        { v: 'evolution', t: '기술 생애주기', why: '경쟁사 선택 — 그 회사 기술들이 성장기인지 성숙기인지 단계 판정' }],
      secondary: [
        { v: 'overview', t: '경영 요약', why: '경쟁 포지션 맵·BCG — 성장률×질 기준 경쟁사 위치 비교' },
        { v: 'overview', t: '🚨 위협 레이더', why: '자사 주력 기술에 새로 진입한 경쟁자 조기 경보' },
        { v: 'power', t: '핵심특허 영향력', why: '경쟁사 선택 — 그 회사의 진짜 핵심 자산(Influence Score) 식별' },
        ...
        { v: 'deepsig', t: '시그널: 분쟁·국가과제', why: '경쟁사 선택 — 심판·소송이 몰린 기술과 국가과제 수행 이력' },
        { v: 'deepsig', t: '시그널: 라이선스·표준·양도', why: '경쟁사 선택 — 실시권·표준특허(SEP)·권리 거래로 본 수익화 활동' },
    ...
    { key: 'fto', icon: '⚖️', label: 'FTO (자유실시조사)',
      ...
      primary: [
        ...
        { v: 'deepsig', t: '시그널: 수명·시장', why: '법적 상태·연차료 생존 — 살아있는 권리와 소멸 특허 구분 (원부 확인 필수)' },
  ...
  Views.competitor = function (content) {
    makeTabs(content, [
      ...
      { label: '기술 DNA', render: function (h) {
        analysisCard({
          analysis: 'company-dna', holder: h,
          title: '경쟁사 기술 DNA Fingerprint (분석 대상 출원인 선택 가능)',
          controls: multiCompanyControls('분석 대상 출원인'),
          help: '12개 지표(집중도 HHI/다양성 entropy/신규진입률/조합다양성/패밀리규모/해외범위/등록유지율/평균피인용/후속출원/공동출원/발명자집중도/최근성장률). 축 값 = 원값 ÷ 비교 모집단 최대값(예: 패밀리 6.4 vs 4.0 → 1.00 vs 0.63) — 상대적으로 낮다고 0으로 붕괴하지 않습니다. 카드 상단의 "회사 추가…" 드롭다운으로 분석 대상 출원인(예: 4개사)을 직접 고르면 그 출원인들만으로 계산·표준화되어, 선택하지 않은 공동출원 상대(대학·파트너 등)가 그래프에 영향을 주지 않습니다 (칩 클릭=제거, 미선택=상위 기업 전체). Hover 에 원값·표준화값 동시 표시.',
          renderOk: function (r, c, setTarget) {
  ...
  Views.manual = function (content) {
    ...
    section('🖱️ 차트 공통 기능',
      ...
      '<li><b>🤖 AI 인사이트 (차트별)</b>: 생성 버튼들은 <b>Settings → LLM 인사이트 설정이 켜져 있을 때</b> 표시됩니다 (전체 사용자 공통 설정). 꺼져 있으면 버튼이 사라진 것처럼 보이는 대신 각 차트의 💡 인사이트 박스에 <b>"🤖 AI 인사이트 켜기"</b> 버튼이 표시되어 클릭 한 번으로 복구할 수 있습니다. 차트가 여러 개인 탭에서 "LLM 인사이트 생성 (차트별)" 버튼을 누르면 <b>차트 선택 체크리스트</b>가 열려 필요한 차트만 골라 생성할 수 있고(불필요한 LLM 호출 절약), 각 차트 아래의 <b>"🤖 이 차트 인사이트"</b> 버튼으로 그 차트 하나만 바로 생성할 수도 있습니다 — 결과는 해당 차트 바로 아래에 표시되고 보관함에 저장됩니다. 생성되는 인사이트는 그 차트의 수치·읽는 법을 근거로 <b>IP Landscape 전문 컨설턴트 수준의</b> 슬라이드 형식 인사이트를 만듭니다 — [슬라이드 제목]→[차트 요지](이 차트가 어떤 목적으로 무엇을 보여주는지 한 줄)→[핵심 메시지]→<b>[심층 해석]</b>(경쟁 구도·기술 수명주기 위치·진입장벽·변곡점·R&amp;D 전략 관점, "관찰 수치→해석→함의" 구조)→[근거 데이터]→[시사점·제언](단기/중기 우선순위 + 후속 분석 제안)→[유의사항]. 뻔한 차트 묘사가 아니라 "이 수치가 왜 중요한가"에 집중하며, 화면 데이터가 뒷받침하지 않는 해석은 쓰지 않도록 지시됩니다 (진행 표시 n/m). 각 차트 아래의 "💡 이 차트의 인사이트"는 LLM 없이 항상 표시되는 규칙 기반 요약입니다. AI 패널에서는 추가 질문(챗)이 가능하고 "웹 검색 포함"을 켜면 출처 링크와 함께 외부 검색을 참고합니다.</li>' +
    ...
    section('❓ 자주 묻는 문제',
      ...
      '<tr><td>분석이 "비활성화"로 표시됨</td><td>Settings → 컬럼 매핑에서 안내된 필수 컬럼을 매핑하세요. \'등록일[KR,JP,…]\' 처럼 국가목록이 붙은 WIPS 헤더와 \'(B1)\' 류 코드는 자동 인식됩니다. <b>기술 대·중·소분류 컬럼이 없어도 IPC/CPC 가 매핑되어 있으면</b> 대분류=섹션(H 전기), 중분류=클래스(H01), 소분류=서브클래스(H01L 반도체 장치)로 자동 분류되어 기술 차트가 활성화됩니다 (라벨에 IPC 코드가 그대로 표기되어 기준을 알 수 있음). 매핑했는데도 안 되면 메시지의 [매핑 진단]과 "예시 값"으로 실제 값 형식을 확인하세요.</td></tr>' +
  ...
  Views.settings = function (content) {
    ...
    var cs = card('💾 분석 스냅샷 — 기존 분석 결과 불러오기',
      ...
      '스냅샷은 팀 공유 자산이라 <b>다른 사람이 저장한 것도 보고 불러올 수 있으며</b>(삭제만 ' +
      '본인/관리자), "내 작업 보기"로 내 것만 좁혀 볼 수도 있습니다. 상단 헤더의 "저장된 분석…" ' +
      '드롭다운에서도 바로 불러올 수 있습니다.');
    ...
    c1.body.appendChild(Ui.el('<div style="color:#647b8d;font-size:11.5px;margin:-4px 0 8px">' +
      '공동출원 집계는 출원인 순위·매트릭스·버블부터 기술 생애주기·경쟁 구도·기업 DNA·' +
      'PAI·만료 절벽·심층 시그널까지 <b>모든 출원인별 집계 차트</b>에 적용됩니다. ' +
      '\'각각 집계\'는 공동출원 1건을 각 공동출원인에게 1건씩 세므로 회사별 합계가 전체 ' +
      '건수를 넘을 수 있습니다 (해당 차트에 안내 표시). 협력 네트워크·공동출원 비율 등 ' +
      '공동출원 자체를 분석하는 화면은 이 설정과 무관하게 원본 그대로 계산됩니다. ' +
      '분석 시작 전에 설정하세요.</div>'));
