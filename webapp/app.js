...
(function () {
  ...
  Views.manual = function (content) {
    ...
    section('🗺️ 메뉴 안내',
      ...
      '<tr><td>⚖️ Patent Power</td><td>핵심특허 영향력, 인용 확산, 청구항 밀집도, 발명자 이동, 출원인·현재권리자 관계(양도 네트워크·순매수/매도), 의미 기반 영향력(임베딩), 권리 중첩 네트워크(임베딩)</td></tr>' +
    ...
    section('🖱️ 차트 공통 기능',
      ...
      '<li><b>🗂️ 인사이트 보관함 → PPT</b>: 생성된 인사이트는 차트 이미지와 함께 작업별로 자동 저장됩니다 (기본은 현재 작업만 표시, 이전 작업은 버튼으로 선택). 항목을 골라 PPT 로 내려받으면 <b>1p 표지, 2p 목차, 이후 슬라이드마다 차트 이미지+인사이트</b>가 들어가고, 카드에 차트가 여러 개면 전부 수록됩니다.</li>' +
